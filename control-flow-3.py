...
cast = cast = dict(zip(cast_names, cast_heights))
...
for key, value in nominated.items():
    for director in value:
        if nom_count_dict.get(director) is None:
...
# Add your code here
for key, value in winners.items():
    for director in winners[key]:
        if win_count_dict.get(director) is None:
            win_count_dict[director] = 1
        else:
            win_count_dict[director] += 1

max_count = max(win_count_dict.values())
